...
const SkillsForm = () => {
  ...
  const handleSpecialitiesChange = (index: number, value: string) => {
    const updated = [...form.specialities];
    updated[index] = value;
    setForm((prev) => ({ ...prev, specialities: updated }));
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          {form.specialities.map((spec, i) => (
            <div key={i} className="flex gap-2">
              <input
                value={spec}
                onChange={(e) => handleSpecialitiesChange(i, e.target.value)}
        ...
        <div className="flex gap-2">
          ...
          {editId && (
            <button
              type="button"
              onClick={resetForm}
              className="w-24 py-2 bg-gray-300 text-gray-800 font-semibold rounded hover:opacity-90 transition"
            >
              Cancel
            </button>
          )}
      ...
      <div className="pt-6 border-t border-[var(--border-color)] space-y-4">
        <h3 className="text-lg font-semibold text-[var(--text-strong)]">Your Skills</h3>
